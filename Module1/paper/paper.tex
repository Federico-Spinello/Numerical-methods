...
\end{enumerate}
È stat anche testata la condizione iniziale "hot", dove, inizialmente, ogni particella aveva spin random. Quello che è stato osservato è che, come atteso, la termalizzazione richiedeva molto più tempo rispetto alla partenza "cold". Nonostante questo, la termalizzazione veniva comunque raggiunta entro i 10000 cluster updates, come è visibile nel grafico \ref{fig:therm}. Le linee tratteggiate colorate rappresentano qualitativamente quando il sistema è termalizzato, con lo schema di colori visibile in tabella \ref{table:colors}. Dopo $\sim 1000$ (linea verticale viola) step l'energia ha raggiunto un regime stazionario, la magnetizzazione, invece, lo raggiunge molto prima. Inoltre, si vede che la termalizzazione viene raggiunta prima se si impone la condizione iniziale fredda.
...
\begin{figure}[H]
    ...
    \caption{Esempio di termalizzazione per $L=200$ a $T=2.4$. Le regioni evidenziate mostrano la fase di termalizzazione (primi 10000 steps, in rosso) e la fase di misura in equilibrio (steps successivi, in verde). In basso uno zoom delle prime 10000 misurazioni.}
    \label{fig:therm}
\end{figure}
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|}
        \hline
        Partenza & Grafico & Termalizzazione\\
        \hline
        Cold & blu & viola\\
        Hot & rosso & arancione\\
        \hline
    \end{tabular}
    \caption{Schema di colori usato per i grafici}
    \label{table:colors}
\end{table}
