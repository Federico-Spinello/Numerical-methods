...
\begin{enumerate}
    ...
    \item È stato osservato che l'energia raggiunge un plateau stazionario dopo $\sim 1000$ aggiornamenti dei cluster, con fluttuazioni statistiche attorno al valore medio.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{../plots/thermalization.png}
...
\begin{equation}
    T_c^{(\chi)} = 2.270 \pm 0.001 \quad \chi^2_{\text{red}}=0.36
\end{equation}
Il valore è in ottimo accordo con la teoria (< 1$\sigma$) e il $\chi^2_{\text{red}} \approx 0.03$ indica che gli errori bootstrap sono realistici e leggermente conservativi. 
\subsection{Scaling di $\chi_{\max}$}

\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{../plots/chi_scaling.png}
    \caption{Scaling log-log di $\chi_{\max}$ vs $L$. Fit: $\gamma/\nu = 1.750 \pm 0.005$ con $\chi^2_{red} = 0.86$.}
    \label{fig:scaling}
\end{figure}

La Figura~\ref{fig:scaling} mostra $\chi_{\max} \sim L^{\gamma/\nu}$ con fit usando \texttt{scipy.optimize.curve\_fit}:
\begin{equation}
    \gamma/\nu = 1.7498 \pm 0.0049, \quad \chi^2_{\text{red}} = 0.86
\end{equation}
Il valore atteso dalla teoria è $\gamma/\nu=1.75$, che è in perfetto accordo con i risultati ottenuti (differenza $\sim 0.01\%$). Questo è confermato anche dal valore del $\chi^2_{\text{red}} = 0.86$, che indica l'ottima qualità del fit essendo $\sim 1$.
...
Questa misura è in eccellente accordo con il valore teorico (errore $\sim 0.03$ mK, $< 1\sigma$), ed è la stima più precisa tra i tre metodi utilizzati. 
...
\begin{equation}
    T_c^C = 2.265 \pm 0.004 \quad \chi^2_{\text{red}} = 0.16
\end{equation}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item FSS picchi $C$: $T_c^C = 2.265 \pm 0.004$ ($\chi^2_{\text{red}} = 0.16$)
    \end{itemize}
    ...
    \item \textbf{Esponente critico}: $\gamma/\nu = 1.750 \pm 0.005$ (esatto: $1.75$), con $\chi^2_{\text{red}} = 0.86$. L'accordo è eccellente, confermando la validità dell'andamento a legge di potenza $\chi_{\max} \sim L^{\gamma/\nu}$.
...
Il metodo bootstrap è risultato ottimale, con $\chi^2_{\text{red}}$ vicini ai valori ideali ($\sim 0.03-0.86$) e stime di $T_c$ in perfetto accordo con il valore teorico.
